/**
 * 5-scenario stress test for the PaperFinder upstream.
 *
 * Hits PAPER_FINDER_URL directly (no app imports - they pull in
 * Next.js + zod + the whole OpenAI client and blow the ts-node heap)
 * and applies the same dedupe + review-filter heuristics the
 * literature-scout agent uses so the numbers match production.
 *
 * Usage:
 *   npx ts-node --transpile-only scripts/test-paper-finder.ts
 */
...
// ── Types (inlined to skip app imports) ─────────────────────────────

type Source = "pubmed" | "arxiv" | "scholar" | "semantic_scholar" | "tavily"
...
async function callPaperFinder(description: string): Promise<any> {
...
// ── Normalisation (mirrors paper-finder.ts) ─────────────────────────
...
function firstStr(obj: any, ...keys: string[]): string | undefined {
  for (const k of keys) {
    const v = obj?.[k]
...
function normalizeSource(raw?: string | null): Source {
...
function normalizeDocs(response: any): NormalizedPaper[] {
  const coll = response?.doc_collection
  const docs: any[] = Array.isArray(coll)
    ? coll
    : (coll?.documents ??
      coll?.docs ??
      coll?.items ??
  ...
  for (const doc of docs) {
    ...
    for (const c of candidates) {
      ...
      abstract =
        firstStr(c, "abstract", "summary", "description", "content") ?? abstract
      publishedDate =
    ...
    out.push({
      ...
      publicationTypes: pubTypes.length ? pubTypes : undefined
    })
  }
  return out
}

function dedupe(papers: NormalizedPaper[]): NormalizedPaper[] {
...
const SCENARIOS: Scenario[] = [
  {
    ...
    alternatives: [
      "xanthan guar synergy viscosity high concentration monoclonal antibody",
  ...
  },
  {
    ...
    description: "Research problem: Improve protein expression",
    alternatives: [
...
async function main() {
  ...
  for (const r of results) {
    ...
    if (r.primary < 8 && r.reviews >= 3) {
      console.log(
        `    → Pool dominated by reviews: ${r.reviews}/${r.unique} are surveys, only ${r.primary} primary research. Review heuristic may be misclassifying.`
      )
      continue
    }
    ...
    if (r.unique < 10) {
      console.log(
        `    → Only ${r.unique} unique papers total. Likely a niche or specificity issue.`
      )
    }
  }
  if (under === 0) {
