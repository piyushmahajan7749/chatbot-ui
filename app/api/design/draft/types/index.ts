...
export interface SearchResult {
  ...
  fullText?: string
  /**
   * Raw publication-type tags from the upstream source (PubMed,
   * Semantic Scholar, etc). Used to drop review articles + meta-
   * analyses so the hypothesis pipeline gets primary research
   * (experiments + data) instead of survey papers (#paper-finder
   * fix). Stays optional so other sources without a typed field
   * still flow through.
   */
  publicationTypes?: string[]
  /**
   * Derived flag - TRUE when the paper looks like a review (publi-
   * cation type tag, title heuristic, or both). Filtered out before
   * the LLM synthesis step.
   */
  isReview?: boolean
  /**
   * 3-4 sentence summary tailored to the user's problem statement +
   * objective. Generated by a separate LLM pass once the candidate
   * list is final, so each paper card reads as "why this paper
   * matters for what you're trying to do" rather than the raw
   * abstract. Falls back to the abstract when generation fails.
   */
