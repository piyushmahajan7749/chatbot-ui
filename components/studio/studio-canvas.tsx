...
export function StudioCanvas({
  ...
  const fetchProjectData = async () => {
    try {
      ...
      // Fetch project details first (needed to validate it exists)
      ...
      // Fetch chats, files, and reports in parallel
      const [allChats, fileData, projectReports] = await Promise.all([
        getChatsByWorkspaceId(actualWorkspaceId),
        getFileWorkspacesByWorkspaceId(actualWorkspaceId),
        profile
          ? getReportsByProject(profile.user_id, actualProjectId)
          : Promise.resolve([])
      ])

      // Filter and enrich chats
      const projectChats = allChats.filter(
        chat => chat.project_id === actualProjectId
      )
      ...
      const projectFiles = fileData.files.filter(
        (file: ProjectFile) => file.project_id === actualProjectId
      )
  ...
  const handleNewChat = () => {
    router.push(
      `/${locale}/${actualWorkspaceId}/chat?projectId=${actualProjectId}`
    )
  }
  ...
  const handleUploadFile = () => {
    ...
    router.push(
      `/${locale}/${actualWorkspaceId}/files?projectId=${actualProjectId}`
    )
  }
  ...
  const getTotalActivity = (): number => {
    ...
    const recentChats = chats.filter(
      chat => new Date(chat.updated_at).getTime() > weekAgo
    ).length
    const recentFiles = files.filter(
      file => file.updated_at && new Date(file.updated_at).getTime() > weekAgo
    ).length
    const recentReports = reports.filter(
      report =>
        report.updated_at && new Date(report.updated_at).getTime() > weekAgo
    ).length
  ...
  if (children) {
    return <div className="h-full bg-slate-50">{children}</div>
  }
      ...
      <div className="flex h-full flex-col bg-slate-50">
        ...
        <div className="border-b border-slate-300 bg-white p-6">
          <div className="flex items-start justify-between">
            ...
            {/* Quick Actions */}
            <div className="ml-0 mt-4 flex flex-col items-start gap-2 sm:ml-6 sm:mt-0 sm:flex-row sm:items-center">
              {onOpenChat && (
                <Button
                  onClick={onOpenChat}
                  variant="outline"
                  className="order-first w-full gap-2 sm:hidden"
                >
                  <IconMessage size={16} />
                  Open Chat
                </Button>
        ...
        <div className="flex-1 overflow-auto p-6">
          <Tabs defaultValue="overview" className="h-full">
            <TabsList className="mb-6 w-full bg-white">
            ...
            <TabsContent value="overview" className="space-y-8">
              ...
              <div className="grid grid-cols-1 gap-4 sm:grid-cols-2 sm:gap-6 lg:grid-cols-4">
                <Card className="rounded-2xl border-blue-100 bg-blue-50 transition-all duration-200 hover:-translate-y-1 hover:shadow-lg">
                ...
                <Card className="rounded-2xl border-emerald-100 bg-emerald-50 transition-all duration-200 hover:-translate-y-1 hover:shadow-lg">
                  <CardContent className="p-6">
                    <div className="flex items-center justify-between">
                      <div>
                ...
                <Card className="rounded-2xl border-orange-100 bg-orange-50 transition-all duration-200 hover:-translate-y-1 hover:shadow-lg">
                  <CardContent className="p-6">
                    <div className="flex items-center justify-between">
                      <div>
                ...
                <Card className="rounded-2xl border-purple-100 bg-purple-50 transition-all duration-200 hover:-translate-y-1 hover:shadow-lg">
                  <CardContent className="p-6">
                    <div className="flex items-center justify-between">
                      <div>
                      ...
                      <div className="max-h-80 overflow-auto">
                        {chats.slice(0, 5).map(chat => (
                          <div
                            ...
                            onClick={() =>
                              router.push(
                                `/${locale}/${actualWorkspaceId}/chat/${chat.id}`
                              )
                            }
                          >
                            <div className="flex items-start gap-3">
                              <div className="mt-0.5 flex size-8 items-center justify-center rounded-lg bg-blue-100">
                                <IconBrain
                                  size={16}
                              ...
                              <div className="min-w-0 flex-1">
                                ...
                                <p className="mt-2 text-xs text-slate-400">
                                  {getTimeAgo(
                                    chat.lastMessageAt || chat.updated_at
                                  )}
                                </p>
                              </div>
                            </div>
                      ...
                      <div className="grid grid-cols-3 gap-4 text-center">
                        <div>
                          <p className="text-2xl font-bold text-slate-800">
                            {chats.length + files.length + reports.length}
                          </p>
              ...
              {chats.length === 0 ? (
                ...
                <div className="grid gap-4">
                  {chats.map(chat => (
                    <Card
                      key={chat.id}
                      className="cursor-pointer rounded-2xl bg-white transition-all duration-200 hover:-translate-y-0.5 hover:shadow-md"
                      onClick={() =>
                        router.push(
                          `/${locale}/${actualWorkspaceId}/chat/${chat.id}`
                        )
                    ...
                    >
                      <CardContent className="p-6">
                        <div className="flex items-start gap-4">
                          ...
                          <div className="min-w-0 flex-1">
                            <h4 className="mb-2 truncate font-medium text-slate-800">
                              {chat.name}
                            </h4>
              ...
              {reports.length === 0 ? (
                ...
                <div className="grid gap-4">
                  {reports.map(report => (
                    <Card
                      key={report.id}
                      className="rounded-2xl bg-white transition-all duration-200 hover:-translate-y-0.5 hover:shadow-md"
                    >
                      <CardContent className="p-6">
                        <div className="flex items-start justify-between">
                          ...
                          <Button
                            ...
                            onClick={() =>
                              router.push(
                                `/${locale}/${actualWorkspaceId}/reports/${report.id}`
                              )
                            }
                          >
                            View Report
